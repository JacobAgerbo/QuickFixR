...
updateCovariate <- function(session){
  ...
  # Batch Normalisation
  updateSelectInput(session, "bnorm_batch", choices = covariates)
...
updateTaxLevel <- function(session){
  ...
  # Batch Normalisation
  updateSelectInput(session, "bnorm_taxlev", choices = tax.name, selected=tax.default)
  ...
  # Differential
  updateSelectInput(session, "diff_taxlev", choices = tax.name, selected=tax.default)
...
observeEvent(input$upload_animalcules,{
  withBusyIndicatorServer("upload_R_data", {
